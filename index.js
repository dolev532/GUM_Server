...
app.get('/', function (request, response) {
	...
	switch(operation) {
		...
		case "Get10TopPressure" : SourceByParam = "SELECT TOP (10) [ProductId],[CustomerId],[Pressure],[Time] FROM [GUM].[dbo].[Pressure] where CustomerId = '" + Param +"'" + "order by Time desc";	break;
		case "Get10TopOnlyPressure" : SourceByParam = "SELECT TOP (10) [Pressure] FROM [GUM].[dbo].[Pressure] where CustomerId = '" + Param +"'" + "order by Time desc";	break;
		case "Get1TopPressure" : SourceByParam = "SELECT TOP (1) [Pressure] FROM [GUM].[dbo].[Pressure] where CustomerId = '" + Param +"'" + "order by Time desc";	break;
		case "Get10TopMeasure" : SourceByParam = "SELECT TOP (10) Measure FROM [GUM].[dbo].[GasMeasure] where CustomerId = '" + Param +"'" + "order by RefreshTime desc";	break;
		case "Get10TopTemperature" : SourceByParam = "SELECT TOP (10) [ProductId],[CustomerId],[temp],[Time] FROM [GUM].[dbo].[Temperature] where CustomerId = '" + Param +"'" + "order by Time desc"; break;
		...
		case "Get1TopMeasure" : SourceByParam = "SELECT TOP (1) Measure FROM [GUM].[dbo].[GasMeasure] where CustomerId = '" + Param +"'" + "order by RefreshTime desc";	break;
		case "AddNewGasMeter" : SourceByParam =  "INSERT INTO [dbo].[GasMeter] ([ProductId] ,[CustomerId],[IsOn] ,[IP] ,[MACAddress],[RefreshTime]) VALUES('" + PArr[0] + "','" + PArr[1] + "','0','" +  PArr[2] + "','" + PArr[3] + "','" + getDateTime() + "')"; break;
		...
		case "DailyConsumption" : SourceByParam = "SELECT Measure FROM [GUM].[dbo].[GasMeasure] where (CONVERT (date, RefreshTime)) = (CONVERT (date, SYSDATETIME())) and CustomerId = '" + Param + "'"; break;
		case "MonthlyConsumption" : SourceByParam = "SELECT TOP (1) Measure FROM [GUM].[dbo].[GasMeasure]  where CustomerId = '" + Param + "' order by RefreshTime desc "; break;
